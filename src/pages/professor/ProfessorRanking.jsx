...
export default function ProfessorRanking() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      try {
        ...
        if (salaIds.length === 0) { setAlunos([]); setTimes([]); return }

        const [{ data: alunosData, error: eAlunos }, { data: timesData, error: eTimes }] = await Promise.all([
          supabase.from('alunos').select('*').in('sala_id', salaIds).order('pontos', { ascending: false }),
          supabase.from('times').select('*').in('sala_id', salaIds).order('pontos', { ascending: false }),
        ])
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-texto/60">Quem está na frente nas suas turmas.</p>
