...
export default function DiretorRanking() {
    ...
    async function carregar() {
      ...
      try {
        ...
        const { data: alunosData, error: eAlunos } = await supabase
          .from('alunos').select('*').eq('escola_id', perfil.escola_id).order('pontos', { ascending: false })
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-texto/60">Quem está na frente na sua escola.</p>

      <div className="mt-6 inline-flex rounded-xl bg-card border p-1">
        <button
          onClick={() => setAba('alunos')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${aba === 'alunos' ? 'bg-azul text-white' : 'text-texto/60 hover:text-white'}`}
        >
          Alunos
        </button>
        <button
          onClick={() => setAba('times')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${aba === 'times' ? 'bg-azul text-white' : 'text-texto/60 hover:text-white'}`}
        >
          Times
        </button>
      </div>
      ...
      {carregando ? (
        ...
        <div className="mt-8 space-y-3">
          {lista.map((item, i) => (
            <div key={item.id} className="rounded-2xl bg-card border p-5 flex items-center gap-5 transition hover:-translate-y-0.5 hover:border-azul/40">
              <div
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-texto/50">{item.salaNome}</div>
